...
describe("Kenshi", function () {
  it("Recovering Kenshi should not work", async function () {
    ...
    await tx(KNS.connect(_owner).transfer(KNS.address, "1000000000000"));

    await expect(
      KNS.recoverERC20(KNS.address, addr1.address, "100000000000")
    ).to.be.revertedWith("Kenshi: cannot recover KNS");
  });
  ...
  it("ERC1363 approve and call should work with data", async function () {
    ...
    await KNS.connect(_owner)["approveAndCall(address,uint256,bytes)"](
      test.address,
      5,
      data
    );
  ...
  it("ERC1363 approve and call should work without data", async function () {
    ...
    const [_owner] = await ethers.getSigners();
    await KNS.connect(_owner)["approveAndCall(address,uint256)"](
      test.address,
      5
    );
  ...
  it("ERC1363 transfer and call should work with data", async function () {
    ...
    await KNS.connect(_owner)["transferAndCall(address,uint256,bytes)"](
      test.address,
      5,
      data
    );
  ...
  it("ERC1363 transfer and call should work without data", async function () {
    ...
    const [_owner] = await ethers.getSigners();
    await KNS.connect(_owner)["transferAndCall(address,uint256)"](
      test.address,
      5
    );
  ...
  it("ERC1363 transfer from and call should work with data", async function () {
    ...
    await tx(KNS.connect(_owner).transfer(_addr1.address, "1000"));
    await tx(KNS.connect(_addr1).approve(_owner.address, "1000"));
  ...
  it("ERC1363 transfer and call should work without data", async function () {
    ...
    await KNS.connect(_owner)["transferFromAndCall(address,address,uint256)"](
      _addr1.address,
      test.address,
      5
    );
